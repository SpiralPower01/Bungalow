...
function closeOverlay() {
  ...
  // On s'assure de fermer aussi le panneau de navigation mobile
  if (DOM.mainNav && DOM.mainNav.classList.contains("nav--visible")) {
...
function openOverlay(clickedElement) {
  ...
  if (targetId === "#apercu") {
    document.body.classList.add("apercu-is-active");
...
export function initOverlay() {
  ...
  DOM.navContainer.addEventListener("click", (event) => {
    ...
    if (clickedElement === activeLink) {
      closeOverlay(); // La fonction closeOverlay fermera maintenant le menu
    } else {
      openOverlay(clickedElement);
      // On ferme le menu mobile dès qu'on ouvre une page
      if (DOM.mainNav && DOM.mainNav.classList.contains("nav--visible")) {
        DOM.mainNav.classList.remove("nav--visible");
        const icon = DOM.navToggleBtn.querySelector("i");
        icon.classList.remove("fa-times");
